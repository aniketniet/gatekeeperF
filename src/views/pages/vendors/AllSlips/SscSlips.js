...
const SscSlips = () => {
  ...
  async function handleParmanentDelete(id) {
    ...
    if (confirmed) {
      try {
        const res = await axios.put(`${import.meta.env.VITE_BASE_URL}admin/ssc/delete/bill/`,{
            id: id
        }, {
          headers: {
            Authorization: `Bearer ${token}`,
        ...
        if (res.status === 200) {
            getSscBills() // Refresh the data after deletion
        } else {
        ...
        <div className="body flex-grow-1">
          ...
          <div className='d-flex justify-content-between'>
                 ...
                 className={services.length === 0 ? "btn  mx-2 disabled" : "btn  mx-2"}
                  ...
                  style={{ textTransform: 'uppercase', fontWeight:'bold',backgroundColor: '#0077ff'  }}
                >
                  Download PDF
                </button>
                </div>
