...
const Material = () => {
  ...
  async function getUsers() {
    ...
    try {
      ...
      let allMaterials = res.data.materials
    ...
    if (confirmed) {
      try {
        const res = await axios.delete(
          `${import.meta.env.VITE_BASE_URL}admin/delete-old-materials`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  async function handleParmanentDelete(id) {
    ...
    if (confirmed) {
      try {
        const res = await axios.delete(`${import.meta.env.VITE_BASE_URL}user/material/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        size: 60,
        Cell: ({ row }) => (
          <Link to={`materialEdit/${row.original._id}`} style={{ textDecoration: 'none' }}>
